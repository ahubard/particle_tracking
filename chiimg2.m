function [chiimg Wip2 IoWP]=chiimg2(img,ip,W,Wip2,range)
...
% img. chiimg is minumum where img and the test image ip are most alike in
...
% 09/22/07 mds update for nonsymetric W and Ip
...
if(~exist('Wip2','var') || isempty(Wip2))  % ip3 can be pre calculated since it does not depend on img
...
IoWP=conv2(img,ip.*W,range);

chiimg=Wip2.*conv2(img.^2,W,range)./IoWP.^2-1;    % best fit ignoring overlap  
